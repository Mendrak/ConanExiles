...
echo Output wil be open in notepad with TeleportPlayer X Y Z + structure class 
set /p x="Enter X coord: "
set /p y="Enter Y coord: "
set /p radius="Enter radius: "
set db=game.db

sqlite3 %db% "SELECT class, id, x, y, z FROM actor_position WHERE class LIKE '/Game/Systems/Building/%%' AND x <= (%x% + %radius%) AND x >= (%x% - %radius%) AND y <= (%y% + %radius%) AND y >= (%y% - %radius%);" >results.txt
